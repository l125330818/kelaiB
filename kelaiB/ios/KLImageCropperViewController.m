...
#define OVERLAYViewAlpha 0.8f

@interface KLImageCropperViewController () {
    BOOL _showAnimating;
    BOOL _hiddeAnimating;
}
...
@implementation KLImageCropperViewController
...
- (void)initView {
    ...
    if (oriHeight < self.cropFrame.size.height) {
        ...
        oriWidth =  oriHeight * (self.originalImage.size.width / self.originalImage.size.height);
    ...
    self.overlayView.alpha = OVERLAYViewAlpha;
...
- (void)initControlBtn {
    ...
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 70, 60)];
    ...
    UIButton *confirmBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.bounds.size.width - 70, 0, 70, 60)];
...
- (void)cancel:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(KLImageCropperDelegate)]) {
...
//        CGFloat scaleRatio = self.showImgView.frame.size.width / self.cropFrame.size.width;
        CGPoint translation = [panGestureRecognizer translationInView:self.showImgView.superview];
        self.showImgView.center = CGPointMake(self.showImgView.center.x + translation.x, self.showImgView.center.y + translation.y);
         [panGestureRecognizer setTranslation:CGPointZero inView:self.showImgView.superview];
...
//        [self.showImgView setCenter:(CGPoint){view.center.x + translation.x * acceleratorX, view.center.y + translation.y * acceleratorY}];
...
- (CGRect)handleBorderOverflow:(CGRect)newFrame {
    ...
    if (CGRectGetMaxX(newFrame) < CGRectGetMaxX(self.cropFrame)){
        newFrame.origin.x = CGRectGetMaxX(self.cropFrame) - newFrame.size.width;
    }
